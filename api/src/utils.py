...
def process_pdf_file(file, text_splitter):
    ...
    for num_page, page in enumerate(docs):
        ...
        for chunk_idx, chunk in enumerate(chunks):
            ...
            metadata.append(
                {
                    "doc": file.filename.split("/")[-1],
                    "page": str(num_page),
                    "chunk": str(chunk_idx),
                    "source": f"f-{file.filename.split('/')[-1]} p-{num_page} c-{chunk_idx}",
                }
            )
...
def process_csv_file(file):
    if file.filename.endswith(".csv"):
        df = pd.read_csv(io.BytesIO(file.file.read()), sep=config.CSV_SEP)
    else:
